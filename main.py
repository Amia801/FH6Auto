...
class FH_UltimateBot(ctk.CTk):
    ...
    def save_config(self):
        ...
        try:
            if hasattr(self, "entry_calc_a"):
                ...
                self.config["calc_d"] = self.entry_calc_d.get().strip()
        except Exception:
            pass
        try:
    ...
    def auto_calculate_pipeline(self):
        ...
        try:
            ...
            val_d = self.entry_calc_d.get().strip()
            sp_per_race = int(val_d) if val_d else 50
        except Exception:
            self.log("输入格式有误，请确保只输入数字！")
            return

        if cost_per_car <= 0 or sp_per_car <= 0 or sp_per_race <= 0:
            self.log("单车成本、单车技能点或每局技能点不能为 0！")
            return

        # 1. 基础转换（总车数 & 总跑图数）
        total_cars = target_cr // cost_per_car
        total_races = (total_cars * sp_per_car) // sp_per_race
        ...
        self.log(
            ...
            f"(每局{sp_per_race}技能点)。分配为: {final_loops} 个大循环, "
            f"每轮跑图 {final_races_per_loop} 次, 动作 {cars_per_loop} 辆。"
        )
        self.save_config()
    ...
    def setup_ui(self):
        ...
        self.entry_share.insert(0, self.config.get("share_code", "338929569"))
        ...
        ctk.CTkLabel(self.calc_frame, text="每局技能点:").pack(side="left", padx=(0, 5))
        self.entry_calc_d = ctk.CTkEntry(self.calc_frame, width=50, height=28)
        self.entry_calc_d.insert(0, self.config.get("calc_d", "50"))
        self.entry_calc_d.pack(side="left", padx=(0, 15))
        ctk.CTkButton(
            self.calc_frame,
            text="计算并应用",
            width=90,
        ...
        self.entry_calc_d.bind("<KeyRelease>", lambda e: limit_len(e, self.entry_calc_d, 2))
        # ==========================================
        ...
        while self.is_running and time.time() - start_time < 300:
            ...
            if passed_screen_1:
                ...
                if pos_continue:
                    ...
                    self.rsleep(3.0) # 点击后过3秒再试，只要有就继续点
                    continue
            ...
            self.rsleep(1.0) # 每次总循环休息1秒，防止CPU占用过高
    ...
    def find_and_remove_consumable_car(self, target_count):
        ...
        while self.sc_count < target_count:
            ...
            self.rsleep(0.8) # 等待“你确定要移除吗”的确认弹窗
